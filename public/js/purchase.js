$(document).ready(function () {
    ...
    $(document).on('change', '.inline_discounts', function () {
        ...
        __write_number(row.find('input.purchase_unit_cost_after_tax'), purchase_after_tax, true)
        row.find('.row_subtotal').text(__currency_trans_from_en(sub_total_after_tax, false, true));
        __write_number(row.find('input.row_subtotal_hidden'), sub_total_after_tax, true);
        row.find('.purchase_product_unit_tax_text').text(__currency_trans_from_en(tax, false, true));
        __write_number(row.find('input.purchase_product_unit_tax'), tax, true);
    ...
    $(document).on('change', '.purchase_cost', function () {
        var row = $(this).closest('tr');
        var quantity = __read_number(row.find('input.purchase_quantity'), true);
        var purchase_before_tax = __read_number($(this), true);
        ...
        row.find('span.row_subtotal').text(__currency_trans_from_en(sub_total_before_tax, false, true));
        __write_number(row.find('input.row_subtotal_hidden'), sub_total_before_tax, true);
    ...
    purchase_table = $('#purchase_table').DataTable({
        ...
        columns: [
            ...
            { data: 'number', name: 'vehicles.number' },
    ...
    grns_table = $('#grns_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[0, 'desc']],
        ajax: '/grns',
        ...
        "fnDrawCallback": function (oSettings) {
            var total_purchase = sum_table_col($('#grns_table'), 'final_total');
            $('#footer_purchase_total').text(total_purchase);

            var total_due = sum_table_col($('#grns_table'), 'payment_due');
            $('#footer_total_due').text(total_due);

            $('#footer_status_count').html(__sum_status_html($('#grns_table'), 'status-label'));

            $('#footer_payment_status_count').html(__sum_status_html($('#grns_table'), 'payment-status-label'));

            __currency_convert_recursively($('#grns_table'));
        },
        createdRow: function (row, data, dataIndex) {
            $(row).find('td:eq(5)').attr('class', 'clickable_td');
        }
    ...
    $('table#grns_table tbody').on('click', 'a.delete-purchase', function (e) {
        e.preventDefault();
        swal({
            title: LANG.sure,
            icon: "warning",
            ...
            success: function (result) {
                $(result).find('.purchase_quantity').each(function () {
                    ...
                    if(!productArray.includes(product_id))
					{
						...
						cells.push(product_id);
					}
                    else
                    {
                        swal({
                            title: "Product already invoiced",
                            });
                    }
                    getID();
...
function update_table_total() {
    ...
    $('#purchase_entry_table tbody').find('tr').each(function () {
        ...
        total_subtotal += __read_number($(this).find('.row_subtotal_hidden'), true);
    });
...
function update_grand_total() {
    ...
    $('span#grand_total_text').text(__currency_trans_from_en(grand_total, true, true));
...
$(document).on('click', 'button.open-AddBookDialog', function(){
    ...
    var id = $(this).data('id');
    $('.tm-tag.tm-tag-info').css('display', 'none');
    $('.tm-tag.tm-tag-info').find('span').text(' ');
    ...
    $(document).on('click', 'button#submit_serial_number-'+id, function(){
        var exitNO = $('input[name=hidden-tags]').val();
        var exit_serial = $('#exit_serial').val();
        var string = exitNO.replace(exit_serial, '');
        $('input[name=hidden-tags]').val(string)
        var seialNO = $('input[name=hidden-tags]').val();
         exitSerial.push(seialNO);
         getID();
        ...
        $.ajax({
            ...
            success:function(e){
                ...
                $("#serial_modal").val(null).trigger("change");
                $('input#product_quantity_'+ row).val(e);

                var raw = $('input#product_quantity_'+ row).closest('tr');
                var quantity = __read_number($('input#product_quantity_'+ row), true);
                var purchase_before_tax = __read_number(raw.find('input.purchase_unit_cost'), true);
                var purchase_after_tax = __read_number(raw.find('input.purchase_unit_cost_after_tax'), true);
        
                //Calculate sub totals
                var sub_total_before_tax = quantity * purchase_before_tax;
                var sub_total_after_tax = quantity * purchase_after_tax;
        
                raw.find('.row_subtotal_before_tax').text(__currency_trans_from_en(sub_total_before_tax, false, true));
                __write_number(raw.find('input.row_subtotal_before_tax_hidden'), sub_total_before_tax, true);
        
                raw.find('.row_subtotal_after_tax').text(__currency_trans_from_en(sub_total_after_tax, false, true));
                __write_number(raw.find('input.row_subtotal_after_tax_hidden'), sub_total_after_tax, true);
...
//     {
//        $('.purchase_quantity').attr('disabled', true);
//        $('.serial-td').css('display', '');
//     }

//     if($(this).val() == 'pending')
//     {
//         $('.purchase_quantity').attr('disabled', false);
//         $('.serial-td').css('display', 'none');
//     }
