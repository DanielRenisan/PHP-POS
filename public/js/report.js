$(document).ready(function () {
    ...
    supplier_report_tbl = $('#supplier_report_tbl').DataTable({
        ...
        columns: [
            ...
            { data: 'due', name: 'due' },
            { data: 'mobile', name: 'contacts.mobile', searchable : true, visible: false}
        ],
    ...
    //Stock report table
    $('#product_st_date_filter').daterangepicker(
        dateRangeSettings,
        function (start, end) {
            $('#product_st_date_filter span').html(start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format));
            stock_report_table.ajax.reload();
        }
    );
    $('#product_st_date_filter').on('cancel.daterangepicker', function(ev, picker) {
        $('#product_st_date_filter').html('<i class="fa fa-calendar"></i> {{ __("messages.filter_by_date") }}');
        stock_report_table.ajax.reload();
    });
    stock_report_table = $('#stock_report_table').DataTable({
        ...
        "ajax": {
            ...
            "data": function (d) {
                ...
                var start = $('#product_st_date_filter').data('daterangepicker').startDate.format('YYYY-MM-DD');
                var end = $('#product_st_date_filter').data('daterangepicker').endDate.format('YYYY-MM-DD');
                d.start_date = start;
                d.end_date = end;
                d.unit_id = $('#unit').val();
            }
        ...
        columns: [
            ...
            { data: 'total_open_stock', name: 'total_open_stock', searchable: false  },
            ...
            { data: 'empty_collect', name: 'empty_collect', searchable: false },
            { data: 'total_balance_stock', name: 'total_balance_stock', searchable: false },
            { data: 'distribution_deposits', name: 'distribution_deposits' , searchable: false },
            { data: 'total_addon_stock', name: 'total_addon_stock', searchable: false },
            { data: 'total_delivery_note', name: 'total_delivery_note', searchable: false },
            { data: 'addon_stock', name: 'addon_stock', searchable: false },
        ...
        "fnDrawCallback": function (oSettings) {
            ...
            $('#footer_sale_total').text(sum_table_col($('#stock_report_table'), 'total-cost'));
        }
    });
    ...
    //Stock report table
    new_stock_report_table = $('#new_stock_report_table').DataTable({
        processing: true,
        serverSide: true,
        "ajax": {
            "url": "/reports/new_stock-report",
            "data": function (d) {
                d.location_id = $('#location_id').val();
                d.category_id = $('#category_id').val();
        ...
        columns: [
            ...
            { data: 'product', name: 'products.name' },
            { data: 'distribution_deposits', name: 'distribution_deposits' , searchable: false },
            { data: 'total_job_stock', name: 'total_job_stock', searchable: false },
            { data: 'empty_stock', name: 'empty_stock', searchable: false },
        ...
        "fnDrawCallback": function (oSettings) {
            __currency_convert_recursively($('#new_stock_report_table'));
            $('#footer_sale_total').text(sum_table_col($('#new_stock_report_table'), 'total-cost'));
        }
    });
    // Array to track the ids of the details displayed rows
    var detailRows = [];

    $('#new_stock_report_table tbody').on('click', 'tr td.details-control', function () {
        var tr = $(this).closest('tr');
        var row = stock_report_table.row(tr);
        var idx = $.inArray(tr.attr('id'), detailRows);
    ...
    // On each draw, loop over the `detailRows` array and show any child rows
    new_stock_report_table.on('draw', function () {
        $.each(detailRows, function (i, id) {
            $('#' + id + ' td.details-control').trigger('click');
        });
    });
    ...
    $('#stock_report_filter_form #location_id, #product_st_date_filter, #stock_report_filter_form #category_id, #stock_report_filter_form #sub_category_id, #stock_report_filter_form #brand, #stock_report_filter_form #unit,#stock_report_filter_form #view_stock_filter').change(function () {
        stock_report_table.ajax.reload();
        new_stock_report_table.ajax.reload();
    ...
    if ($('#sr_date_filter').length == 1) {
        ...
        sr_asset_report = $('#sr_asset_report').DataTable({ 
            processing: true,
            serverSide: true,
            "ajax": {
                "url": "/assets?type=1",
            ...
            columns: [
                { data: 'asset_type', name: 'asset_type' , "searchable": false},
                { data: 'ref_no', name: 'ref_no' },
                { data: 'name', name: 'name'},
                { data: 'value', name: 'value', "searchable": false },
                { data: 'issue_date', name: 'issue_date', "searchable": false },
                { data: 'approve', name: 'au.first_name'},
                { data: 'created', name: 'cu.first_name'},
		    ],
        });
        ...
        sr_sales_commission_report = $('table#sr_sales_with_commission_table').DataTable({
            ...
            aaSorting: [[0, 'desc']],
            "ajax": {
                "url": "/reports/staff-commission-report",
                "data": function (d) {
                    var start = $('input#sr_date_filter').data('daterangepicker').startDate.format('YYYY-MM-DD');
                    var end = $('input#sr_date_filter').data('daterangepicker').endDate.format('YYYY-MM-DD');
            ...
            columns: [
                {data: 'service_staff_name', name: 'service_staff_name'},
                                ...
                                {data: 'sale_commission', name: 'sale_commission', searchable: false}
            ],
            columnDefs: [
                {
                    'searchable': false,
    ...
    product_sell_report = $('table#product_sell_report_table').DataTable({
        ...
        columns: [
            ...
            { data: 'sub_bill_price', name: 'sub_bill_price', searchable: false }
...
function updateProfitLoss() {
    ...
    var loader = __fa_awesome();
    $('.opening_stock, .total_transfer_shipping_charges, .closing_stock, .total_sell, .total_delivery, .total_purchase, .total_expense, .net_profit, .total_adjustment, .total_recovered, .total_sell_discount, .total_purchase_discount').html(loader);
    ...
    $.ajax({
        ...
        data: data,
        success: function (data) {
            var total = data.total_sell + data.balance;
            console.log(data.total_sell + data.balance);
