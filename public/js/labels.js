$(document).ready(function () {
    ...
    // gift voucher label print
    $('button#voucher_labels_preview').click(function () {
        $.ajax({
            method: "POST",
            url: '/labels/voucher_preview',
            dataType: "json",
            // data: $('form#preview_setting_form').serialize(),
            success: function (result) {
                if (result.success) {
                    $('div.display_voucher_label_div').removeClass('hide');
                    $('div#voucher_preview_box').html(result.html);
                    __currency_convert_recursively($('div#voucher_preview_box'));
                } else {
                    toastr.error(result.msg);
                }
    ...
    $(document).on('click', 'button#print_voucher_label', function () {
        var is_chrome = function () { return Boolean(window.chrome); }

        if (is_chrome) {
            var print_div = document.getElementById("voucher_preview_body");
            ...
            print_area.print();
            // print_area.close();
            setTimeout(function () { print_area.close(); }, 1000);
        }
        else {
            var print_div = document.getElementById("voucher_preview_body");
            var print_area = window.open();
