	...
	$("#search_product").autocomplete({
		...
		.autocomplete("instance")._renderItem = function (ul, item) {
			if (item.enable_stock == 1 && item.qty_available <= 0) {
				...
				string += ' (' + item.sub_sku + ')' + "<br> Stock: " + stock + ' (Out of stock) </li>';
				...
				string += ' (' + item.sub_sku + ')' + "<br> Stock: " + item.qty_available + "</div>";
				return $("<li>")
					.append(string)
					.appendTo(ul);
			}
...
function pos_product_row(variation_id, quantity) {
	...
	if (add_via_ajax) {
		...
		$.ajax({
			...
			data: { product_row: product_row, customer_id: customer_id, is_direct_sell: is_direct_sell, quantity: quantity, is_card:true },
			dataType: "json",
			success: function (result) {
				if (result.success) {
					console.log(result);
