    ...
    if($( "#search_product_for_srock_adjustment" ).length > 0){
		...
		$( "#search_product_for_srock_adjustment" ).autocomplete({
			source: function(request, response) {
	    		$.getJSON("/vehicle-products/list", { location_id: $('#location_id').val(), vehicle_id: $('select#vehicle_id').val(), term: request.term }, response);
	  			},
			minLength: 2,
...
function stock_transfer_product_row(variation_id){
	...
	$.ajax({
		method: "POST",
		url: "/stock-adjustments/get_vehicle_row",
		data: {row_index: row_index, variation_id: variation_id, location_id: location_id, vehicle_id: vehicle_id},
		dataType: "html",
		success: function(result){
            console.log(result);
...
function get_stock_transfer_details(rowData){
	var div = $('<div/>')
        ...
        .text( 'Loading...' );
    $.ajax( {
        url: '/vehicle-transfers/' + rowData.DT_RowId,
        dataType: 'html',
