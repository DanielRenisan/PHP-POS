$(document).ready(function () {
	$(document).on('ifChecked', 'input#is_previous', function(){
        $('div.previous-amount').show();
      document.getElementById("gift_voucher_amount").required = true;
	  document.getElementById("is_previous").value = 1;
    });
	$(document).on('ifUnchecked', 'input#is_previous', function(){  
		$('div.previous-amount').hide();
      	document.getElementById("gift_voucher_amount").required = false;
		  document.getElementById("is_previous").value = null;
	});
	...
	$("#search_product").autocomplete({
		...
		select: function (event, ui) {
			if (ui.item.enable_stock != 1 || ui.item.qty_available > 0) {
				...
				// pos_product_row(ui.item.variation_id, 1);
				$.ajax({
					method: "POST",
					url: '/getPromotionRow',
					data: { variation_id: ui.item.variation_id, location_id: location_id },
					dataType: "json",
					success: function (result) {
						if (result.showModel == true) {
							swal({
								title: 'Promotion Alert',
								text: 'If customer buy ' + result.buy_product_quantity + ' of this product, They can get ' + result.get_promotion_quantity + ' of ' + result.get_product_name + ', is customer agree with this promotion?',
								icon: "info",
								buttons: true,
								dangerMode: false,
							}).then((ifAgree) => {
								if (ifAgree) {
									pos_product_row(result.variation_id, result.buy_product_quantity);
									setTimeout(
										function () {
											pos_product_row(result.get_promotion_id, result.get_promotion_quantity);
										}, 500);
								}
								else {
									pos_product_row(result.variation_id, 1);
								}
							});
						} else {
	...
	$('table#pos_table tbody').on('change', 'input.pos_quantity', function () {
        ...
        var unit_discount_amount = __read_number(tr.find('input.pos_line_discount'));
		var no_discount_total = entered_qty * unit_price_inc_tax;
		var line_total = entered_qty * unit_price_inc_tax;
		var type = tr.find('input.pos_line_discount_type').val();
        var line_discount = 0;
		if(type == 'fixed')
		{
			line_discount = unit_discount_amount;
		}
		else
		{
			line_discount = (unit_discount_amount /100 ) * line_total;
		}
        if(line_discount != '' || line_discount != null)
        {
            line_total = line_total - line_discount.toFixed(2);
        }

        
		__write_number(tr.find('input#discount'), line_discount, false, 2);
		__write_number(tr.find('input.pos_line_total'), line_total, false, 2);
		__write_number(tr.find('input#line_unit_price_quantity'), no_discount_total, false, 2);
		tr.find('span.pos_line_total_text').text(__currency_trans_from_en(line_total, true));
		tr.find('span.pos_line_discount_text').text(__currency_trans_from_en(line_discount, true));
		pos_total_row();
	});
	...
	$('table#pos_table tbody').on('click', 'i.pos_remove_row', function () {
		swal({
			...
			if (willDelete) {
				var id = $(this).attr('id');
				console.log(id)
				let productString = String(id + ',');
				var product = document.getElementById("exit_product_id").value.split(productString).pop();
				document.getElementById("exit_product_id").value = product;
				// $(this).parents('#tr' + id).remove();

				$.ajax({
					async: false,
					// global: false,
					method: "POST",
					url: '/check_delete_product_promotion',
					dataType: "json",
					data: { product_id: id },
					success: function (data) {
						if (data.isPromotion == true) {
							$('i.pos_remove_row').parents('#tr' + data.buy_product_id).remove();
							$('i.pos_remove_row').parents('#tr' + data.get_product_id).remove();
						}
						else {
							$('i.pos_remove_row').parents('#tr' + data.buy_product_id).remove();
						}
	...
	$('button#pos-cancel').click(function () {
		swal({
			...
			if (willDelete) {
				reset_pos_form();
				window.location.href = "http://localhost/pos/public/pos/create";
			}
		});
	});
	...
	$('button.pos-sale-return-modifier-btn').click(function () {

		var data = $('#return_pos_sell_form').serialize();
		...
		$.ajax({
			...
			success: function (result) {
				if (result.success == 1) {
					...
					toastr.success(result.msg);
                    window.location.href= "http://localhost/pos/public/pos/return/modifier/"+result.id;
				} else {
					toastr.error(result.msg);
				}
			}
	...
	$('button.pos-sale-return-btn').click(function () {
		if ($('table#pos_table tbody').find('.product_row').length <= 0) {
			toastr.warning(LANG.no_products_added);
			return false;
		}

		var exit_product_id = $('input#exit_product_id').val();
		var productArray = exit_product_id.split(',').map(function(item) {
			return parseInt(item, 10);
		});

		var previous_product_id = $('input#previous_product_id').val();
		var previousArray = previous_product_id.split(',').map(function(item) {
			return parseInt(item, 10);
		});
		var newArray = productArray.filter(function (value) {
			return !Number.isNaN(value);
		});
		let Equals = newArray.every((item)=>previousArray.includes(item));
		console.log(Equals, newArray, previousArray);
		if(!Equals)
		{
			toastr.warning('Please add correct products');
			reset_pos_form();
			return false;
		}
			var data = $('#return_pos_sell_form').serialize();
	...
	$('button.pos-express-finalize').click(function () {
		...
		if ($('input#gift_voucher_amount_modal').val()) {
			...
			$.ajax({
				...
				data: { code: $('input#gift_voucher_amount_modal').val() },
				dataType: "json",
				success: function (data) {
					console.log(data);
				},
		...
		if ($('input#loyalty_points_amount_modal').val()) {
			$.ajax({
				async: false,
				// global: false,
				method: "PUT",
				url: '/updateloyaltyPoints',
				data: { customer_id: $('input#loyalty_customer_modal').val(), mobile: $('input#loyalty_mobile_modal').val(), points: $('input#loyalty_points_amount_modal').val(), },
				dataType: "json",
				success: function (data) {
					console.log(data);
				},
		...
		if (pay_method == 'card') {
			...
			$('div#cash_details_modal').modal('show');
		}
		else if (pay_method == 'credit') {
			$('div#credit_details_modal').modal('show');
		}
		else {
			//
		}
	...
	$('button#pos-save-crdit').click(function () {

		$('div#credit_details_modal').modal('hide');
		pos_form_obj.submit();
	});

	// on save cash
	$('button#pos-save-cash').click(function () {

		$('div#cash_details_modal').modal('hide');
		pos_form_obj.submit();
	});
	...
	$('button#add-payment-row').click(function () {
		...
		$.ajax({
			...
			data: { row_index: row_index, is_return:$('#return').val() },
			dataType: "html",
			success: function (result) {
				if (result) {
	...
	//Update Gift Voucher
	$('button#posEditGiftVoucherUpdate').click(function () {
		let code = $('input#gift_voucher_amount_modal').val()
		$('#gift_voucher_code').val(code);
		//Close modal
		$.ajax({
			async: false,
			method: "POST",
			url: '/updatePrevious',
			data: { code: $('input#gift_voucher_amount_modal').val(), amount: $('input#gift_voucher_amount').val(), is_previous: $('input#is_previous').val(), issuedDate :  $('input#issuedDate').val()
			, expiryDate :  $('input#expiryDate').val()},
			dataType: "json",
			success: function (data) {
				if (data.success == 1) {
					$('div#posEditGiftVoucherModal').modal('hide');
		...
		__write_number($('input#gift_voucher'), __read_number($('input#gift_voucher_amount_modal')));
		// $('span.text_gift_voucher').text(__currency_trans_from_en(__read_number($('input#gift_voucher')), true))
		// console.log(__read_number($('input#gift_voucher')));

		pos_total_row();
	});

	//Update Loyalty
	$('button#posEditLoyaltyUpdate').click(function () {
		//Close modal
		$('div#posEditLoyaltyModal').modal('hide');

		//Update values
		// $('input#discount_type').val('fixed');
		__write_number($('input#loyalty_amount'), __read_number($('input#loyalty_points_amount_modal')));
		$('span.text_loyalty_amount').text(__currency_trans_from_en(__read_number($('input#loyalty_points_amount_modal')), true));
		// console.log(__read_number($('input#gift_voucher_amount_modal')));
	...
	$('input#loyalty_customer_modal').change(function () {

		if ($('input#loyalty_customer_modal')) {
			$.ajax({
				async: false,
				// global: false,
				method: "POST",
				url: '/getAvailablePoints',
				data: { customer_id: $('input#loyalty_customer_modal').val(), mobile: $('input#loyalty_mobile_modal').val() },
				dataType: "json",
				success: function (data) {
					if (data.statusCode == 200) {
						$('span#availableLoyaltyPoints').text(data.result);
					}
					else if (data.statusCode == 404) {
						$('span#availableLoyaltyPoints').text(0);
	...
	$('input#loyalty_customer_modal').keypress(function (e) {
		var key = e.which;
	...
	//Update Loyalty by mobile number
	$('input#loyalty_mobile_modal').change(function () {

		if ($('input#loyalty_mobile_modal')) {
			$.ajax({
				async: false,
				// global: false,
				method: "POST",
	...
	$('input#loyalty_mobile_modal').keypress(function (e) {
		var key = e.which;
		if (key == 13)  // the enter key code
		{
			if ($('input#loyalty_mobile_modal')) {
	...
	//Show product list.
	get_product_suggestion_list($("select#product_category").val(), $('select#product_group').val(), $('input#location_id').val(), null);
	$("select#product_category, select#product_group").on("change", function (e) {
		var location_id = $('input#location_id').val();
	...
	$(document).on('click', 'ul.pagination_ajax a', function (e) {
		...
		var group_id = $("select#product_group").val();

		var url = $(this).attr('href');
		get_product_suggestion_list(category_id, group_id, location_id, url);
	});
	...
	$(document).on('click', 'div.product_box', function () {
		...
		if ($('input#location_id').val() == '') {
			...
			$.ajax({
				method: "POST",
				url: '/getPromotionRow',
				data: { variation_id: $(this).data('variation_id'), location_id: location_id },
				dataType: "json",
				success: function (result) {
					if (result.isAvailableQty != false) {
						if (result.showModel == true) {
							swal({
								title: 'Promotion Alert',
								text: 'If customer buy ' + result.buy_product_quantity + ' of this product, They can get ' + result.get_promotion_quantity + ' of ' + result.get_product_name + ', is customer agree with this promotion?',
...
function get_product_suggestion_list(category_id, group_id, location_id, url = null) {
	if (url == null) {
...
function pos_product_row(variation_id, quantity) {
	...
	if (add_via_ajax) {
		...
		$.ajax({
			...
			data: { product_row: product_row, customer_id: customer_id, is_direct_sell: is_direct_sell, quantity: quantity },
			dataType: "json",
			success: function (result) {
				if (result.success) {
                    var exit_product_id = $('input#exit_product_id').val();
                    ...
                    if(!productArray.includes(result.product.product_id))
					{
                        $('table#pos_table tbody').append(result.html_content).find('input.pos_quantity');
                        //increment row count
                        $('#row_quantity_modal').modal('show');
						$('#row_quantity_modal').on('shown.bs.modal', function () {
							$('#quantity-input').focus();
							$('#quantity-input').select();
						})
						var input = document.getElementById("quantity-input");
						var disInput = document.getElementById("discount-input");
						...
						unitInput.addEventListener("keypress", function(event) {
							if (event.key === "Enter") {
								event.preventDefault();
								$('button.pos-popup-submit').focus();
								var this_row = $('table#pos_table tbody').find("tr").last();
								var quantity = $('input.pos_popup_quantity').val();
								var discount = $('input.pos_popup_discount').val();
								var discount_type = $('input#discount-type-input').val();
								var subtotal = $('input.pos-popup-total').val();
								var unit = subtotal/quantity;
								this_row.find('input.pos_quantity').val(quantity);
								this_row.find('input.pos_line_discount').val(discount);
								this_row.find('input.pos_line_discount_type').val(discount_type);
								this_row.find('#pos_quantity').trigger('change');
								this_row.find('input#pos_line_total').val(subtotal);
								this_row.find('input.row_discount_amount').val(discount);
								this_row.find('select.row_discount_type option[value="'+discount_type+'"]').prop("selected", true);
								this_row.find('input.row_discount_amount').trigger('change');
								this_row.find('#pos_line_total').trigger('change');
								this_row.find('input.pos_unit_price_inc_tax').val(unit);
								this_row.find('input.pos_unit_price_inc_tax').trigger('change');
								this_row.find('span.pos_line_total_text').text(__currency_trans_from_en(subtotal, true));
								__write_number(this_row.find('input.pos_unit_price_inc_tax'), unit);
								__write_number(this_row.find('input#pos_line_total'), subtotal);
								console.log(subtotal);
								pos_total_row();
								$('#row_quantity_modal').modal('hide');
							}
						});
						get_product_discount(result.product.product_id);
                        $('#quantity_modal_product_name').text(result.product.product_name+' '+result.product.sub_sku);
						$('input.pos_popup_unit_price').val(result.product.default_sell_price);
                        $('input.pos-popup-total').val(result.product.default_sell_price);
                        $('span.pos-popup-total').text(result.product.default_sell_price);
                        $('input.current_product_id').val(result.product.product_id);
                        $('input.pos_popup_quantity').val($('input.pos_quantity').val())
                        $('input#product_row_count').val(parseInt(product_row) + 1);
                        cells.push(result.product.product_id);
                    }
                    else
					{
						var quanty = parseInt($('input.row_pos_quantity_'+result.product.product_id).val()) + 1;
                        $('input.row_pos_quantity_'+result.product.product_id).val(quanty);
                        this_row.find('input.row_pos_quantity_'+result.product.product_id).trigger('change');
                    }
                    getID();
...
function get_product_discount(product_id)
{
	if(product_id)
	{
		...
		$.ajax({
			method: "POST",
			url: "/get_product_discount",
			data: { product_id: product_id, location_id : location_id },
			dataType: "json",
			success: function (result) {
				$("#discount-input").val(result.amount);
				$("#discount-type-input").val(result.type);
				$("#discount-input").trigger('change');
			}
		});
	}
...
function getID(){
	...
	{ 
		str += cells[i] + ",";  // adding each element with key number to variable
	} 
	
	document.getElementById('exit_product_id').value = str;
}


// $(document).on('keypress', 'button.pos-popup-submit', function (e) {
//     // if (e.which === 13) {
//     var this_row = $('table#pos_table tbody').find("tr").last();
//     var quantity = $('input.pos_popup_quantity').val();
//     var discount = $('input.pos_popup_discount').val();
// 	var subtotal = $('input.pos-popup-total').val();
// 	var unit = $('input.pos_popup_unit_price').val();
//     this_row.find('input.pos_quantity').val(quantity); 
//     this_row.find('input.pos_line_discount').val(discount);
//     this_row.find('#pos_quantity').trigger('change');
// 	this_row.find('input#pos_line_total').val(subtotal);
...
//     // }
// });
$(document).on('click', 'button.pos-popup-submit', function (e) {
    // if (e.type === 'click') {
    var this_row = $('table#pos_table tbody').find("tr").last();
    var quantity = $('input.pos_popup_quantity').val();
    var discount = $('input.pos_popup_discount').val();
	var subtotal = $('input.pos-popup-total').val();
	var unit = $('input.pos_popup_unit_price').val();
	var return_type = $('select#select_return_type').val();
	var discount_type = $('input#discount-type-input').val();
	var line_total = quantity * unit;
    this_row.find('input.pos_quantity').val(quantity); 
    this_row.find('input.pos_line_discount').val(discount);
	this_row.find('input.pos_line_discount_type').val(discount_type);
    this_row.find('#pos_quantity').trigger('change');
	...
	this_row.find('#pos_line_total').trigger('change');
	this_row.find('input.pos_unit_price_inc_tax').val(unit);
	this_row.find('input#line_unit_price_quantity').val(line_total);
	this_row.find('input.pos_unit_price_inc_tax').trigger('change');
	this_row.find('span.pos_line_total_text').text(__currency_trans_from_en(subtotal, true));
	__write_number(this_row.find('input.pos_unit_price_inc_tax'), unit);
	__write_number(this_row.find('input#pos_line_total'), subtotal);
	this_row.find('input.line_return-type').val(return_type);
	this_row.find('span.line_return_type_text').text(return_type);
	console.log(subtotal);
	pos_total_row();
	$('input.pos_popup_quantity').val(' ');
	$('input.pos_popup_quantity').val(1);
    $('#row_quantity_modal').modal('hide');
	
    // }
});

$('input.pos_popup_quantity').change(function (e) {
    var total = 0;
    var unit = __read_number($('#row_quantity_modal').find('input.pos_popup_unit_price'));
    var discount = __read_number($('#row_quantity_modal').find('input.pos_popup_discount'));
    total = unit * $(this).val();
	var type = $('#row_quantity_modal').find('input#discount-type-input').val();
    if(discount != '' || discount != 0)
    {
		...
		{
			sub_discount = (discount/100) * total;
		}
        total = total - sub_discount;
    }
    $('span.pos-popup-total').text(total.toFixed(2));
	$('input.pos-popup-total').val(total.toFixed(2));
});
$('input.pos_popup_discount').change(function (e) {
    var total = 0;
    var unit = __read_number($('#row_quantity_modal').find('input.pos_popup_unit_price'));
    var discount =  __read_number($('#row_quantity_modal').find('input.pos_popup_discount'));
    var quantity = __read_number($('#row_quantity_modal').find('input.pos_popup_quantity'));
	var type = $('#row_quantity_modal').find('input#discount-type-input').val();
    total = unit * quantity;
	var sub_discount = 0;
	if(type == 'fixed')
...
$('input.pos_popup_unit_price').change(function (e) {
    var total = 0;
    var unit = $(this).val();
    var discount = __read_number($('#row_quantity_modal').find('input.pos_popup_discount'));
	var quantity = __read_number($('#row_quantity_modal').find('input.pos_popup_quantity'));
...
function pos_total_row() {
	...
	__write_number($('input#final_total_input'), price_total);
	__write_number($('span#total_payable'), price_total);
...
function calculate_billing_details(price_total) {
	...
	var loyality_points = __read_number($('input#loyalty_amount'));
	var gift_amount = __read_number($('input#gift_voucher'));

	var total_payable = price_total + order_tax - discount + shipping_charges-loyality_points-gift_amount;
	...
	$('input#customer_total_paying').change(function () {

		customer_total_payable = total_payable;

		customer_total_paying = $('input#customer_total_paying').val();

		customer_change_return = customer_total_paying - customer_total_payable;

		$('span.change_return_span').text(__currency_trans_from_en(customer_change_return, true));
		$('input.payable_changes').val(customer_change_return);
	});

	$('input#customer_total_paying').keypress(function (e) {
		var key = e.which;
		if (key == 13)  // the enter key code
		{
			customer_total_payable = total_payable;
...
function pos_discount(total_amount) {
	...
	function returnAmount() {
		if (__read_number($('input#gift_voucher_amount_modal'))) {
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data.statusCode == 200) {
						...
						$('input#gift_voucher').val(data.result);
					}
					else if (data.statusCode == 404) {
						toastr.error(data.result);
					}
			...
			// $('input#gift_voucher_amount_modal').val("");
			return parseInt(tempResponse);
	...
	//ca
	function returnLoyaltyPoints() {
		if (__read_number($('input#loyalty_customer_modal')) || __read_number($('input#loyalty_mobile_modal')) || __read_number($('input#loyalty_points_amount_modal'))) {
			var tempResponse = null;
			$.ajax({
				async: false,
				// global: false,
				method: "POST",
				url: '/searchByCustomer',
				data: { customer_id: $('input#loyalty_customer_modal').val(), mobile: $('input#loyalty_mobile_modal').val(), points: $('input#loyalty_points_amount_modal').val(), },
				dataType: "json",
				success: function (data) {
	...
	function returnInvoiceDiscountAmount() {
		if (__read_number($('input#location_id')) || __read_number($('select#customer_id'))) {
			var location = $('input#location_id').val();
			var customer = $('select#customer_id').val();
			var tempResponse = null;
			$.ajax({
				async: false,
				// global: false,
				method: "POST",
				url: '/getPromotionDiscount',
				dataType: "json",
				data: { location_id: location, customer_id: customer },
				success: function (data) {
					if (data.success == 1) {
						tempResponse = data.invoice_discount_amount;
					}
				},
	...
	var basic_calculation_amount = __read_number($('#discount_amount'));

	var line_discount = 0;

	var gift_calculation_amount = returnAmount();

	var loyalty_calculation_amount = returnLoyaltyPoints();

	var invoice_discount_amount = returnInvoiceDiscountAmount();


	var gift_discount = __calculate_gift_voucher(gift_calculation_amount, total_amount);
	var basic_discount = __calculate_amount(calculation_type, basic_calculation_amount, total_amount);
	var loyalty_discount = __calculate_loyalty(loyalty_calculation_amount, total_amount);
	var invoice_discount = __calculate_promotion_discount(invoice_discount_amount, total_amount);



	if (line_discount != null && basic_discount == null && loyalty_discount == null) {
		discount = line_discount;
	} else if (gift_discount == null && basic_discount != null && loyalty_discount == null) {
		discount = basic_discount;
	}
	else if (gift_discount == null && basic_discount == null && loyalty_discount != null) {
		// discount = loyalty_discount;
	}
	else if (line_discount != null && basic_discount != null && loyalty_discount == null) {
		discount = basic_discount + line_discount;
	}
	else if (gift_discount == null && basic_discount != null && loyalty_discount != null) {
		discount = basic_discount;
	}
	...
	if (invoice_discount != null) {
		discount = discount + invoice_discount;
	}

	if (gift_discount != null) {
		$('input#gift_voucher').val(gift_discount);
		$('span.text_gift_voucher').text(__currency_trans_from_en(gift_discount, true))
	}

	if (loyalty_discount != null) {
		$('input#loyalty_amount').val(loyalty_discount);
	}
...
$("#posEditGiftVoucherModal").on("show.bs.modal", function () {
	$('#gift_voucher_amount_modal').val('');
	$('#is_previous').val('');
	$('#gift_voucher_amount').val('');
});

$("#posEditDiscountModal").on("show.bs.modal", function () {
	$('#discount_amount_modal').val('');
});
$("#posEditLoyaltyModal").on("show.bs.modal", function () {
	$('#loyalty_customer_modal').val('');
	$('#loyalty_points_amount_modal').val('');
	$('#availableLoyaltyPoints').text(0);
});

$("#row_quantity_modal").on("show.bs.modal", function () {
	$('.input-group#quantity-input').val(' ');
	$('.input-group#quantity-input').val(1);
});
...
function reset_pos_form() {
	...
	$('span.text_loyalty_amount', 'span.text_gift_voucher').text(0);
	...
	console.log(11);
	$("#exit_product_id").val('');
	$("#exit_product_id").trigger("change");
}
...
$(document).ready(function(){
    function typeChange(){
		...
		$('.line_return-type-'+row).val(value);

		if(value == 'exchange')
		{
			$('.invoice-modifier-btn').show();
		}
    }
    $(".select_return_type").on("change", typeChange);
});