$(document).ready( function () {
    ...
    if($( "#search_product_for_srock_adjustment" ).length > 0){
		...
		$( "#search_product_for_srock_adjustment" ).autocomplete({
			source: function(request, response) {
	    		$.getJSON("/transfer-products/list", { location_id: $('#location_id').val(), term: request.term }, response);
	  			},
			minLength: 2,
    ...
    stock_transfer_table = $('#stock_transfer_table').DataTable({
		processing: true,
		serverSide: true,
		ajax: '/additional/stock-transfers',
		columnDefs: [ {
			"targets": 6,
			"orderable": false,
			"searchable": false
		...
		columns: [
            ...
            { data: 'final_total', name: 'final_total', visible: false},
...
function stock_transfer_product_row(variation_id){
	...
	$.ajax({
		method: "POST",
		url: "/stock-adjustments/get_delivery_row",
...
function get_stock_transfer_details(rowData){
	var div = $('<div/>')
        ...
        .text( 'Loading...' );
    $.ajax( {
        url: '/additional/stock-transfers/' + rowData.DT_RowId,
        dataType: 'html',
