            ...
            <ul class="perfect-scrollbar relative h-[calc(100vh-80px)] space-y-0.5 overflow-y-auto overflow-x-hidden p-4 py-0 font-semibold"
                ...
                <li class="nav-item">
                    <ul>
                        ...
                        {{-- Dynamic Stations (KOT/BOT/DOT/COT/...) --}}
                        @php
                            $sidebarStations = \Illuminate\Support\Facades\Schema::hasTable('stations')
                                ? \App\Models\Station::active()->orderBy('display_order')->orderBy('name')->get()
                                : collect();
                        @endphp
                        @if($sidebarStations->isNotEmpty() && (auth()->user()->can('station.view') || auth()->user()->can('station.display.view')))
                        <li class="menu nav-item">
                            <button type="button" class="nav-link group {{in_array($request->segment(1), ['stations', 'station-display']) ? 'active' : '' }}"
                                @click="activeDropdown === 'stations' ? activeDropdown = null : activeDropdown = 'stations'">
                                <div class="flex items-center">
                                    <i class="fa fa-th-list"></i>
                                    <span class="text-black ltr:pl-3 rtl:pr-3 dark:text-[#506690] dark:group-hover:text-white-dark">Stations</span>
                                </div>
                                <div class="rtl:rotate-180" :class="{'!rotate-90' : activeDropdown === 'stations'}">
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M9 5L15 12L9 19" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                </div>
                            </button>
                            <ul x-cloak x-show="activeDropdown === 'stations'" x-collapse class="sub-menu text-gray-500">
                                @can('station.view')
                                <li class="{{ $request->segment(1) == 'stations' ? 'active' : '' }}">
                                    <a href="{{ route('stations.index') }}">Manage Stations</a>
                                </li>
                                @endcan
                                @can('station.display.view')
                                @foreach($sidebarStations as $sidebarStation)
                                    <li class="{{ $request->segment(2) == $sidebarStation->slug ? 'active' : '' }}">
                                        <a href="{{ route('station-display.index', $sidebarStation->slug) }}">
                                            {{ $sidebarStation->name }} ({{ $sidebarStation->code }})
                                        </a>
                                    </li>
                                @endforeach
                                @endcan
                                ...
                                <ul x-cloak x-show="activeDropdown === 'business'" x-collapse
                                    ...
                                    @if(auth()->user()->can('printer.view') || auth()->user()->can('printer.create'))
                                        <li class="{{ $request->segment(1) == 'printers' ? 'active' : '' }}">
                                            <a href="{{ route('printers.index') }}">Printers</a>
                                        </li>
                                    @endif
                                </ul>
                            </li>
