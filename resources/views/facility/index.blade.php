        ...
        <div class="grid grid-cols-1 gap-4 pt-5">
            <div x-data="facilityList">
                <div class="panel border-[#e0e6ed] px-0 dark:border-[#1b2e4b]">
                    <div class="px-5" x:data="facilityList">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div class="mb-5 flex items-center gap-2">
                                @can('room-facility.create')    
                                    <button  class="btn btn-success gap-2" style="gap:.5rem !important;display: flex;" @click="openModal = true">
                                ...
                                <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                                    ...
                                    <div class="flex items-start justify-center min-h-screen px-4"
                                        ...
                                        <div x-transition x-transition.duration.300
                                            ...
                                            <div class="heading">
                                                <h2 class="m-0">Add Room Facility</h2>
                                            </div>
                                            <div class="p-5">
                                            {!! Form::open(['url' => action('RoomFacilityController@store'), 'method' => 'post', 'class' => 'space-y-5', 'id' => 'facilityi6_add_form' ]) !!}
                                                    <div>
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                                    ...
                                    <div class="flex items-start justify-center min-h-screen px-4"
                                        ...
                                        <div x-transition x-transition.duration.300
                                            class="panel border-0 p-0 rounded-lg overflow-hidden my-8 w-full max-w-lg" style="border-radius:.5rem;border-width: 0;">
                                            <div class="heading">
                                                <h2 class="m-0">Edit  Room Facility</h2>
                                            </div>
                                            <div class="p-5">
                                                {!! Form::open(['url' => action('RoomFacilityController@update'),'class'=>'space-y-5', 'method' => 'PUT', 'id' => 'facility_edit_form' ]) !!}
                                                    <div>
                                                        {!! Form::label('name', __('Facility') . ':*') !!}
                                                        {!! Form::text('name', null, ['class' => 'form-input','required']); !!}
                                                    </div>
                                                    <input id="edit-id" type="hidden" name="facility_id">
                                                    <div class=" flex justify-end items-center mt-3">
                                                        <button type="button" class="btn btn-outline-danger"
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', "table#facility_table tbody tr td", function() {
            ...
            $(document).on('click', 'button.edit-btn', function() {
                ...
                const edit_url = "{{action('RoomFacilityController@show', ['ID'])}}".replace('ID', id);
                $.ajax({
                    method: "GET",
                    url: edit_url,
                    dataType: "json",
                    success: function (result) {
                        console.log(result)
                        $('form#facility_edit_form #edit-id').val(result.id);
                        $('form#facility_edit_form').find('#name').val(result.name);
                    }
                });
            });
        });
        ...
        $(document).on('change', '#check-box', function()
        {
            ...
            {   
                ...
                $(document).on('click', 'button.delete-button', function() {
                    const delete_url = "{{action('RoomFacilityController@destroy')}}";
                    $.ajax({
                        method: "GET",
                        url: delete_url,
                        dataType: "json",
