...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    ...
    <div>
        <ul class="flex space-x-2 rtl:space-x-reverse">
            ...
            <li>
                <a href="{{action('PurchaseWastageController@index')}}" class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">Purchase Wastage</a>
            </li>
            <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                <span>Create Purchase Wastage</span>
            </li>
        </ul>
        ...
        <div class="grid grid-cols-1 gap-4 pt-5">
            <div class="panel border-[#e0e6ed] px-0 dark:border-[#1b2e4b]">
                ...
                {!! Form::open(['url' => action('PurchaseWastageController@store'), 'method' => 'post', 'id' => 'add_purchase_return_form', 'files' => true ]) !!}
                        <div class="purchase-return-div">
                            @include('purchase_wastage.partial.row_data_form', ['row_count' => 0])    
                        </div>

                        <div class="buttons"
                            style="display: flex; align-items: center; justify-content: flex-end; gap: 10px; margin-top: 50px;">
                            <button type="submit" data-status="received" class="btn btn-primary btn-lg btn-flat  gap-2">CREATE</button>
                            <button type="button" class="btn btn-dark btn-lg btn-flat gap-2">Close</button>
                        </div>
                    {!! Form::close() !!}
                </div>
