...
<div class="animate__animated p-6 no-print no-print" :class="[$store.app.animation]">
    ...
    <div>
        <ul class="flex space-x-2 rtl:space-x-reverse">
            ...
            <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1"><a  href="{{action('PurchaseController@index')}}">
                    ...
                    <div style="padding: 15px;">
                        ...
                        @php 
                            ...
                            <div class="mb-6 flex flex-wrap items-center justify-center gap-4 lg:justify-end" style="padding-right:15px">
                                <button type="button" class="btn btn-primary gap-2 print-invoice" data-href="{{action('PurchaseController@printInvoice', [$transaction->id])}}">
                                ...
                                <a href="{{action('PurchaseController@index')}}" class="btn btn-dark"
                                        >BACK</a>
                            </div>
                        </div>
                            <div class="p-5">
                                    ...
                                    <div class="flex flex-col flex-wrap justify-between gap-6 lg:flex-row">
                                        <div class="flex-1">
                                            <div class="space-y-1 text-white-dark">
                                                <div>Issue For:</div>
                                                <div class="font-semibold text-black dark:text-white"><span>{{$transaction->first_name}}</span></div>
                                                <div><span>{{$transaction->address_one}}</span></div>
                                                <div><span>{{$transaction->email}}</span></div>
                                                <div><span>{{$transaction->mobile_no}} {{$transaction->telephone_no ? '/'. $transaction->telephone_no : ''}}</span></div>
                                            </div>
                                        </div>
                                        <div class="flex flex-col justify-between gap-6 sm:flex-row lg:w-2/3">
                                            <div class="xl:1/3 sm:w-1/2 lg:w-2/5">
                                                ...
                                                <div class="mb-2 flex w-full items-center justify-between">
                                                    ...
                                                    <div><span>{{$transaction->total_quantity}}</span></div>
                                                </div>
                                            ...
                                            <div class="xl:1/3 sm:w-1/2 lg:w-2/5">
                                                ...
                                                <div class="mb-2 flex w-full items-center justify-between">
                                                    <div class="text-white-dark">Total Paid :</div>
                                                    <div><span>Rs. {{ number_format($total_paid, 2) }}</span></div>
                                                </div>
                                                <div class="mb-2 flex w-full items-center justify-between">
                                                    <div class="text-white-dark">Remaining Amount :</div>
                                                    <div><span>Rs. {{ number_format($due_amount, 2) }}</span></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <tbody>
                                                @foreach($purchase_lines as $line)
                                                    <tr>
                                                        ...
                                                        <td class="ltr:text-right rtl:text-left display_currency" data-currency_symbol="true">{{ $line['discount'] }}</td>
                                    ...
                                    <div class="mt-6 grid grid-cols-1 px-4 sm:grid-cols-2">
                                        ...
                                        <div class="space-y-2 ltr:text-right rtl:text-left">
                                            <div class="flex items-center">
                                                ...
                                                <div class="w-[37%]"><span>Rs. {{ number_format($sub_total, 2) }}</span></div>
                                            </div>
                                            <div class="flex items-center">
                                                <div class="flex-1">Tax</div>
                                                <div class="w-[37%]"><span>Rs. {{ number_format($transaction->tax_amount, 2) }}</span></div>
                                            </div>
                                            <div class="flex items-center">
                                                <div class="flex-1">Discount</div>
                                                <div class="w-[37%]"><span>Rs. {{ number_format($discount_amount, 2) }}</span></div>
                                            </div>
                                            <div class="flex items-center text-lg font-semibold">
                                                <div class="flex-1">Grand Total</div>
                                                <div class="w-[37%]"><span>Rs. {{ number_format($grand_total, 2) }}</span></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
