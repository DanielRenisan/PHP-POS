    ...
    <div>
        <ul class="flex space-x-2 rtl:space-x-reverse">
            ...
            <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                <span>Expence Category Lists</span>
            </li>
        </ul>
                    ...
                    <div class="px-5" x:data="sizeList">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div class="mb-5 flex items-center gap-2">
                                @can('category.create')    
                                    <button type="button" class="btn btn-success gap-2" @click="openModal = true">
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                                ...
                                <div class="flex items-start justify-center min-h-screen px-4"
                                    ...
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="heading">
                                            <h2 class="m-0">Add Category</h2>
                                        </div>
                                        <div class="p-5">
                                        {!! Form::open(['url' => action('CategoryController@store'), 'method' => 'post', 'class' => 'space-y-5', 'id' => 'facilityi6_add_form' ]) !!}
                                                <div>
                                                ...
                                                @if(!empty($parent_categories))
                                                    ...
                                                    <div class="hide" id="parent_cat_div">
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                                ...
                                <div class="flex items-start justify-center min-h-screen px-4"
                                    ...
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="p-5">
                                            {!! Form::open(['url' => action('CategoryController@update'),'class'=>'space-y-5', 'method' => 'PUT', 'id' => 'cat_edit_form' ]) !!}
                                                    <div>
                                                        {!! Form::label('name', __( 'Category Name' ) . ':*') !!}
                                                        {!! Form::text('name', null, ['class' => 'form-input', 'required', 'placeholder' => __( 'Category Name' )]); !!}
                                                    </div>
                                                    ...
                                                    <div>
                                                        {!! Form::label('short_code', __( 'Category Code' ) . ':') !!}
                                                        {!! Form::text('short_code', null, ['class' => 'form-input', 'placeholder' => __( 'Category Code' )]); !!}
                                                        ...
                                                        <div>
                                                            <div class="checkbox">
                                                            <label>
                                                                {!! Form::checkbox('add_as_sub_cat', 1, null,[ 'class' => 'toggler', 'data-toggle_id' => 'parent_cat_div' , 'id' => 'add_as_sub_cat']); !!} @lang( 'Add as Sub Category' )
                                                            </label>
                                                            </div>
                                                        </div>
    ...
    $(document).ready(function () {
        ...
        facility_table = $('#facility_table').DataTable({
            ...
            columns: [
                ...
                { data: 'selected_parent', name: 'selected_parent' }
            ],
        ...
        $(document).on('click', "table#facility_table tbody tr td", function() {
            ...
            $(document).on('click', 'button.edit-btn', function() {
                ...
                const edit_url = "{{action('CategoryController@show', ['ID'])}}".replace('ID', id);
                $.ajax({
                    method: "GET",
                    url: edit_url,
                    dataType: "json",
                    success: function (result) {
                        console.log(result)
                        $('form#cat_edit_form #edit-id').val(result.id);
                        $('form#cat_edit_form').find('#name').val(result.name);
                        $('form#cat_edit_form').find('#short_code').val(result.short_code);
                        if(result.parent_id !== 0)
                        {
                            $('form#cat_edit_form').find('#add_as_sub_cat').prop('checked', true);
                            $('form#cat_edit_form').find('#parent_cat_div').show();
                        }
                        else
                        {
                            $('form#cat_edit_form').find('#add_as_sub_cat').prop('checked', false);
                            $('form#cat_edit_form').find('#parent_cat_div').hide();
                        }
                        $('form#cat_edit_form').find('#parent_id').val(result.parent_id);
                    }
                });
            });
        });
        $(document).on('ifChecked', 'input#add_as_sub_cat', function(){  
            $('form#cat_edit_form').find('#parent_cat_div').show();
        });
        $(document).on('ifUnchecked', 'input#add_as_sub_cat', function(){  
            $('form#cat_edit_form').find('#parent_cat_div').hide();
        });
        var searchIDs = [];
        $(document).on('change', '#check-box', function()
        {
            ...
            {   
                ...
                $(document).on('click', 'button.delete-button', function() {
                    ...
                    const delete_url = "{{action('CategoryController@destroy')}}";
                    $.ajax({
                        method: "GET",
                        url: delete_url,
                        dataType: "json",
