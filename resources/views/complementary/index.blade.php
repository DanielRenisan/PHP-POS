                    ...
                    <div class="px-5" x:data="sizeList">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div class="mb-5 flex items-center gap-2">
                                @can('complementary.delete')
                                    <button type="button" class="btn btn-danger gap-2 delete-button" @click="deleteRow()" data-href = "{{action('ComplementaryController@destroy')}}">
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                                ...
                                <div class="flex items-start justify-center min-h-screen px-4"
                                    ...
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="heading">
                                            <h2 class="m-0">Add Complementary</h2>
                                        </div>
                                        <div class="p-5">
                                        {!! Form::open(['url' => action('ComplementaryController@store'), 'method' => 'post', 'class' => 'space-y-5', 'id' => 'facilityi6_add_form' ]) !!}
                                                <div>
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                                ...
                                <div class="flex items-start justify-center min-h-screen px-4"
                                    ...
                                    <div x-transition x-transition.duration.300
                                        class="panel border-0 p-0 rounded-lg overflow-hidden my-8 w-full max-w-lg">
                                        <div class="heading">
                                            <h2 class="m-0">Edit Complementary</h2>
                                        </div>
                                        <div class="p-5">
                                            {!! Form::open(['url' => action('ComplementaryController@update', ['ID']),'class'=>'space-y-5', 'method' => 'PUT', 'id' => 'call_edit_form' ]) !!}
                                            <div>
                                                    {!! Form::label('room_type', __('Room Type') . ':*') !!}
                                                    <select class="form-input" name="room_type" required id="seachable-select" x-model="itemToEdit.room_type">
                                                        <option>Please Select One</option>
                                                        @foreach($room_types as $key => $room_type)
                                                            <option value="{{$room_type}}">{{$room_type}}</option>
                                                        @endforeach
...
<script type="text/javascript">
    document.addEventListener('alpine:init', () => {
        Alpine.data('sizeList', () => ({
            selectedRows: [],
            items:  <?php echo $complementaries; ?>,
            searchText: '',
            openModal: false,
            editModal: false,
            viewModal: false,
