                    ...
                    <div class="px-5" x:data="sizeList">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div class="mb-5 flex items-center gap-2">
                                @can('printer.delete')
                                    <button type="button" class="btn btn-danger gap-2 delete-button" @click="deleteRow()" data-href = "{{action('PrinterController@destroy')}}">
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                                ...
                                <div class="flex items-start justify-center min-h-screen px-4"
                                    ...
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="heading">
                                            <h2 class="m-0">Add Printer</h2>
                                        </div>
                                        <div class="p-5">
                                        {!! Form::open(['url' => action('PrinterController@store'), 'method' => 'post', 'id' => 'add_printer_form' ]) !!}
                                        <div x-data="{ connType: 'network', printerType: 'receipt' }" class="grid grid-cols-3 gap-5">
                                            <div>
                                                {!! Form::label('name', __('Printer Name') . ':*') !!}
                                                {!! Form::text('name', null, ['class' => 'form-input', 'required', 'placeholder' => __('Printer name')]); !!}
                                            </div>
                                            <div>
                                                {!! Form::label('printer_type', __('Printer Type') . ':*') !!}
                                                {!! Form::select('printer_type', $printer_types ?? [], 'receipt', ['class' => 'form-select', 'x-model' => 'printerType']); !!}
                                                <span class="help-block text-xs text-gray-500">Receipt = customer invoice. Station = KOT/BOT/etc.</span>
                                            </div>
                                            <div x-show="printerType === 'station'">
                                                {!! Form::label('station_id', __('Station') . ':*') !!}
                                                {!! Form::select('station_id', $stations ?? ['' => 'None'], null, ['class' => 'form-select']); !!}
                                                <span class="help-block text-xs text-gray-500">Which station this printer serves.</span>
                                            </div>
                                            <div>
                                                {!! Form::label('connection_type', __('Connection Type') . ':*') !!}
                                                {!! Form::select('connection_type', $connection_types, 'network', ['class' => 'form-select', 'x-model' => 'connType']); !!}
                                            </div>
                                            <div x-show="connType === 'network'">
                                                {!! Form::label('ip_address', __('IP Address') . ':*') !!}
                                                {!! Form::text('ip_address', null, ['class' => 'form-input', 'placeholder' => '192.168.1.100']); !!}
                                            </div>
                                            <div x-show="connType === 'network'">
                                                {!! Form::label('port', __('Port') . ':*') !!}
                                                {!! Form::text('port', 9100, ['class' => 'form-input']); !!}
                                            </div>
                                            <div x-show="connType === 'windows' || connType === 'linux'">
                                                {!! Form::label('path', __('Device Path') . ':*') !!}
                                                {!! Form::text('path', null, ['class' => 'form-input', 'placeholder' => 'e.g. /dev/usb/lp0 or LPT1']); !!}
                                                <span class="help-block text-xs text-gray-500">Windows: <code>LPT1</code> / <code>COM1</code> / <code>\\HOST\Share</code> &nbsp; Linux: <code>/dev/usb/lp0</code> or CUPS queue name</span>
                                            </div>
                                            <div x-show="connType === 'file'">
                                                {!! Form::label('path', __('Output Folder (optional)')) !!}
                                                {!! Form::text('path', null, ['class' => 'form-input', 'placeholder' => 'leave blank for storage/app/printer-output/{slug}']); !!}
                                                <span class="help-block text-xs text-gray-500">Receipts saved to disk for testing — no real printer needed. Each print creates a <code>.txt</code> and <code>.bin</code> file.</span>
                                            </div>
                                            <div>
                                                {!! Form::label('char_per_line', __('Characters Per Line')) !!}
                                                {!! Form::number('char_per_line', 42, ['class' => 'form-input']); !!}
                                                <span class="help-block text-xs text-gray-500">Typically 42 for 80mm, 32 for 58mm.</span>
                                            </div>
                                            <div class="flex items-center gap-6 pt-6">
                                                <label class="flex items-center gap-2 cursor-pointer">
                                                    {!! Form::hidden('is_active', 0) !!}
                                                    {!! Form::checkbox('is_active', 1, true, ['class' => 'form-checkbox']) !!}
                                                    <span>Active</span>
                                                </label>
                                                <label class="flex items-center gap-2 cursor-pointer">
                                                    {!! Form::hidden('is_default', 0) !!}
                                                    {!! Form::checkbox('is_default', 1, false, ['class' => 'form-checkbox']) !!}
                                                    <span>Default</span>
                                                </label>
                                            </div>
                                        </div>
