                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                                ...
                                <div class="flex items-start justify-center min-h-screen px-4"
                                    ...
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="heading">
                                            <h2 class="m-0">Add Floor</h2>
                                        </div>
                                        <div class="p-5">
                                        {!! Form::open(['url' => action('FloorController@store'), 'method' => 'post', 'class' => 'space-y-5', 'id' => 'facilityi6_add_form' ]) !!}
                                                <div>
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                                ...
                                <div class="flex items-start justify-center min-h-screen px-4"
                                    ...
                                    <div x-transition x-transition.duration.300
                                        class="panel border-0 p-0 rounded-lg overflow-hidden my-8 w-full max-w-lg" style="border-radius:.5rem;border-width: 0;">
                                        <div class="heading">
                                            <h2 class="m-0">Edit Floor</h2>
                                        </div>
                                        <div class="p-5">
                                            {!! Form::open(['url' => action('FloorController@update', ['ID']),'class'=>'space-y-5', 'method' => 'PUT', 'id' => 'call_edit_form' ]) !!}
                                                <div>
                                                    {!! Form::label('name', __('Floor Name') . ':*') !!}
                                                    {!! Form::text('name', null, ['class' => 'form-input','required']); !!}
                                                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        var ajax_url = "{{action('FloorController@index')}}";
        ...
        $(document).on('click', "table#facility_table tbody tr td", function() {
            ...
            $(document).on('click', 'button.edit-btn', function() {
                ...
                const edit_url = "{{action('FloorController@show', ['ID'])}}".replace('ID', id);
                $.ajax({
                    method: "GET",
                    url: edit_url,
                    dataType: "json",
                    success: function (result) {
                        console.log(result)
                        $('form#call_edit_form #edit-id').val(result.id);
                        $('form#call_edit_form').find('#name').val(result.name);
                    }
                });
            });
        });
        ...
        $(document).on('change', '#check-box', function()
        {
            ...
            {   
                ...
                $(document).on('click', 'button.delete-button', function() {
                    const delete_url = "{{action('FloorController@destroy')}}";
                    $.ajax({
                        method: "GET",
                        url: delete_url,
                        dataType: "json",
