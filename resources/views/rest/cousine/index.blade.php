                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="heading">
                                            <h2 class="m-0">Add Cousine</h2>
                                        </div>
                                        <div class="p-5">
                                            <form enctype="multipart/form-data" id="signupForm" class="needs-validation"
                                                method="POST" action="{{ route('cousine.store') }}">
                                                @csrf
                                                <div class="grid grid-cols-1 gap-4 pt-5">
                                                    <div>
                                                        <label for="name">Name</label>
                                                        <input id="name" type="text" placeholder="Cousine Name" name="name"
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        class="panel border-0 p-0 rounded-lg overflow-hidden my-8 w-full max-w-lg"  style="border-radius:.5rem;border-width: 0;">
                                        <div class="heading">
                                            <h2 class="m-0">Edit Cousine</h2>
                                        </div>
                                        <div class="p-5">
                                            <form enctype="multipart/form-data" id="cat_edit_form" class="needs-validation"
                                                novalidate="" method="POST" action="{{ route('cousine.update') }}">
                                                @csrf
                                                ...
                                                <div class="grid grid-cols-1 gap-4 pt-5">
                                                    ...
                                                    <div style="display: flex; align-items: center; gap: 10px;">
                                                        <img style="width: 50px; height: 50px; margin: 0; border-radius: 5px; object-fit: cover;"
                                                            :src="itemToEdit.image" alt="Current Icon" class="w-24 h-24 mb-2">
                                                        <input class="form-input" id="Image" type="file" name="image"
                                                            x-model="itemToEdit.image" accept="image/*"
...
<script>
    ...
    $(document).ready(function () {
        var ajax_url = "{{action('Rest\CousineController@index')}}";
        ...
        $(document).on('click', "table#facility_table tbody tr td", function() {
            ...
            $(document).on('click', 'button.edit-btn', function() {
                ...
                const edit_url = "{{action('Rest\CousineController@show', ['ID'])}}".replace('ID', id);
                $.ajax({
                    method: "GET",
                    url: edit_url,
                    dataType: "json",
        ...
        $(document).on('change', '#check-box', function()
        {
            ...
            {   
                ...
                $(document).on('click', 'button.delete-button', function() {
                    const delete_url = "{{action('Rest\CousineController@delete')}}";
                    $.ajax({
                        method: "GET",
                        url: delete_url,
                        dataType: "json",
