                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="heading">
                                            <h2 class="m-0">Add Employee Type</h2>
                                        </div>
                                        <div class="p-5">
                                            <form id="signupForm" class="needs-validation" method="POST"
                                                action="{{ route('employee_type.store') }}">
                                                @csrf
                                                <div class="grid grid-cols-1 gap-4 pt-5">
                                                    <div>
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        class="panel border-0 p-0 rounded-lg overflow-hidden my-8 w-full max-w-lg">
                                        <div class="heading">
                                            <h2 class="m-0">Edit Employee Type</h2>
                                        </div>
                                        <div class="p-5">

                                            <form id="employee_type_form" class="needs-validation" novalidate="" method="POST"
                                                action="{{ route('employee_type.update') }}">
                                                @csrf
                    ...
                    <div class="category-table" style="position: relative;padding: 15px;">
                        ...
                        <div class="table-responsive">
                            <table  id="example" class="custom-table display nowrap" style="width:100%">
                                ...
                                <tfoot>
                                    <tr>
                                        <td colspan="11">{{$employeeTypes->links('vendor.pagination.tailwind')}}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
...
<script>
    ...
    document.getElementById('pagination').onchange = function() { 
        window.location = "{!! $employeeTypes->url(1) !!}&items=" + this.value; 
    }; 
    ...
    $(document).ready(function () {
        $(document).on('click', "table#example tbody tr td", function() {
            ...
            $(document).on('click', 'button.edit-btn', function() {
                ...
                const edit_url = "{{action('Rest\EmployeeTypeController@show', ['ID'])}}".replace('ID', id);
                $.ajax({
                    method: "GET",
                    url: edit_url,
                    dataType: "json",
                    success: function (result) {
                        $('form#employee_type_form #type_id').val(result.id);
                        $('form#employee_type_form').find('#Name').val(result.name);
                        if(result.status == 'Active')
                        {
                            $('form#employee_type_form').find('#status-input').prop('checked', true);
                        }
                        else
                        {
                            $('form#employee_type_form').find('#status-input').prop('checked', false);
                        }
                    }
                });
            });
        ...
        $(document).on('change', '#check-box', function()
        {
            ...
            {   
                ...
                const delete_url = "{{action('Rest\EmployeeTypeController@delete')}}";
                $.ajax({
                    method: "GET",
                    url: delete_url,
                    dataType: "json",
    ...
    $(document).on('click', 'discard-btn', function() {
        $('form#employee_type_form #type_id').val(' ');
        $('form#employee_type_form').find('#Name').val(' ');
    });
