...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    <div>
        ...
        <div class="grid grid-cols-1 gap-4 pt-5">
            <div x-data="productVariation">
                <div class="panel border-[#e0e6ed] px-0 dark:border-[#1b2e4b]">
                    <div class="px-5" x:data="productVariation">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div class="mb-5 flex items-center gap-2">
                                @can('product-variation.create')
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="heading">
                                            <h2 class="m-0">Add Product Variation</h2>
                                        </div>
                                        <div class="p-5">
                                            <form id="signupForm" class="needs-validation" method="POST"
                                                action="{{ route('product_variation.store') }}">
                                                @csrf
                                                <div style="display: flex; flex-direction: column; gap: 10px;">

                                                    <div>
                                                        <label for="bookingDate">Type</label>
                                                        <select class="form-select text-white-dark" id="type" name="type"
                                                            required>
                                                    ...
                                                    <div style="margin-top: 20px">
                                                        <label class="flex items-center cursor-pointer">
                                                            <input type="checkbox" class="form-checkbox" name="decimal_value"
                                                            value="1" />
                                                            <span class="text-white-dark">Is Decimal</span>
                                                        </label>
                                                    </div>
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        class="panel border-0 p-0 rounded-lg overflow-hidden my-8 w-full max-w-lg"   style="border-radius:.5rem;border-width: 0;">
                                        <div class="heading">
                                            <h2 class="m-0">Edit Product Variation</h2>
                                        </div>
                                        <div class="p-5">

                                            <form id="variant_edit_form" class="needs-validation" method="POST"
                                                action="{{ route('product_variation.update') }}">
                                                @csrf
                                                ...
                                                <div style="display: flex; flex-direction: column; gap: 10px;">
                                                    ...
                                                    <div style="margin-top: 20px">
                                                        <label class="flex items-center cursor-pointer">
                                                            <input type="checkbox" class="form-checkbox" name="decimal_value" id="decimal-input"
                                                            value="1" />
                                                            <span class="text-white-dark">Is Decimal</span>
                                                        </label>
                                                    </div>
...
<script>
    ...
    $(document).ready(function () {
        var ajax_url = "{{action('Rest\ProductVaritationController@index')}}";
        ...
        $(document).on('click', "table#facility_table tbody tr td", function() {
            ...
            $(document).on('click', 'button.edit-btn', function() {
                ...
                const edit_url = "{{action('Rest\ProductVaritationController@show', ['ID'])}}".replace('ID', id);
                $.ajax({
                    method: "GET",
                    url: edit_url,
                    dataType: "json",
                    success: function (result) {
                        console.log(result)
                        $('form#variant_edit_form #edit-id').val(result.id);
                        $('form#variant_edit_form').find('#type').val(result.type);
                        $('form#variant_edit_form').find('#name').val(result.name);
                        if(result.status == 'Active')
                        {
                            $('form#variant_edit_form').find('#status-input').prop('checked', true);
                        }
                        else
                        {
                            $('form#variant_edit_form').find('#status-input').prop('checked', false);
                        }
                        if(result.decimal_value == 1)
                        {
                            $('form#variant_edit_form').find('#decimal-input').prop('checked', true);
                        }
                        else
                        {
                            $('form#variant_edit_form').find('#decimal-input').prop('checked', false);
                        }
                    }
                });
            });
        ...
        $(document).on('change', '#check-box', function()
        {
            ...
            {   
                ...
                $(document).on('click', 'button.delete-button', function() {
                    const delete_url = "{{action('Rest\ProductVaritationController@delete')}}";
                    $.ajax({
                        method: "GET",
                        url: delete_url,
                        dataType: "json",
