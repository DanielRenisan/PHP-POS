...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    <div>
        ...
        <div class="grid grid-cols-1 gap-4 pt-5">
            <div x-data="tablelist">
                <div class="panel border-[#e0e6ed] px-0 dark:border-[#1b2e4b]">
                    <div class="px-5" x:data="{tablelist, checkAllCheckbox: false, selectedRows: [] }">
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="heading">
                                            <h2 class="m-0">Add Table</h2>
                                        </div>
                                        <div class="p-5">
                                            <form enctype="multipart/form-data" id="signupForm" class="needs-validation"
                                                method="POST" action="{{ route('table.store') }}">
                                                @csrf
                                                <div style="display: flex; flex-direction: column; gap: 10px;">
                                                    <div>
                                                        <label for="tableLocation">Table Location</label>
                                                        <select class="form-select text-white-dark" name="tablelocation_id"
                                                            id="TableLocationID" required>
                                                            <option value="" disabled selected>Select Location</option>
                                                            @foreach ($location_list as $tl)
                                                    ...
                                                    <div>
                                                        ...
                                                        <input id="TableName" type="text" placeholder="Name" name="tablename"
                                                            class="form-input" required />
                                                    </div>

                                                    <div>
                                                        <label for="tableCode">Table Code</label>
                                                        <input id="TableCode" type="text" placeholder="Table Code"
                                                            name="tablecode" class="form-input" required />
                                                    </div>
                                                    <div>
                                                        <label for="supplier">Department<span>*</span></label>
                                                        {!! Form::select('department_id', $business_locations , null, ['class' => 'form-input', 'id' => 'department_id', 'required',
                                                        'placeholder' => __('Please Select')]); !!}
                                                    </div>
                                                    <div>
                                                        <label for="chairNo">Chair No</label>
                                                        <input id="ChairNo" type="text" placeholder="Product Name"
                                                            name="chairno" class="form-input" required />
                                                    </div>
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        class="panel border-0 p-0 rounded-lg overflow-hidden my-8 w-full max-w-lg" style="border-radius:.5rem;border-width: 0;">
                                        <div class="heading">
                                            <h2 class="m-0">Edit Table</h2>
                                        </div>
                                        <div class="p-5">
                                            <form enctype="multipart/form-data" id="table_edit_form" class="needs-validation"
                                                method="POST" action="{{ route('table.update') }}">
                                                @csrf
                                                ...
                                                <div style="display: flex; flex-direction: column; gap: 10px;">
                                                    ...
                                                    <div>
                                                        <label for="tableCode">Table Code</label>
                                                        <input id="TableCode" type="text" placeholder="Product Code"
                                                            name="tablecode" class="form-input"
                                                            required />
                                                    </div>
                                                    <div>
                                                        <label for="supplier">Department<span>*</span></label>
                                                        {!! Form::select('department_id', $business_locations , null, ['class' => 'form-input', 'id' => 'location_id', 'required',
                                                        'placeholder' => __('Please Select')]); !!}
                                                    </div>
                                                    <div>
                                                        <label for="chairNo">Chair No</label>
                                                        <input id="chairno" type="text" placeholder="Product Name"
                                                            name="chairno" class="form-input"
                                                            required />
                                                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        var ajax_url = "{{action('Rest\TableController@index')}}";
        ...
        $(document).on('click', "table#facility_table tbody tr td", function() {
            ...
            $(document).on('click', 'button.edit-btn', function() {
                ...
                const edit_url = "{{action('Rest\TableController@show', ['ID'])}}".replace('ID', id);
                $.ajax({
                    method: "GET",
                    url: edit_url,
                    dataType: "json",
                    success: function (result) {
                        console.log(result)
                        $('form#table_edit_form #edit-id').val(result.id);
                        $('form#table_edit_form').find('#TableLocationID').val(result.table_location_id);
                        $('form#table_edit_form').find('#TableName').val(result.table_name);
                        $('form#table_edit_form').find('#TableCode').val(result.table_code);
                        $('form#table_edit_form').find('#location_id').val(result.department_id);
                        $('form#table_edit_form').find('#chairno').val(result.chair_no);
                        $('form#table_edit_form').find('img').attr("src", result.icons);
                        if(result.status == 'Active')
                        {
                            $('form#table_edit_form').find('#status-input').prop('checked', true);
                        }
                        else
                        {
                            $('form#table_edit_form').find('#status-input').prop('checked', false);
                        }
                    }
                });
            });
        ...
        $(document).on('change', '#check-box', function()
        {
            ...
            {   
                ...
                $(document).on('click', 'button.delete-button', function() {
                    const delete_url = "{{action('Rest\TableController@delete')}}";
                    $.ajax({
                        method: "GET",
                        url: delete_url,
                        dataType: "json",
