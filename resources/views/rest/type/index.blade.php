        ...
        <div class="grid grid-cols-1 gap-4 pt-5">
            <div x-data="typelist">
                <div class="panel border-[#e0e6ed] px-0 dark:border-[#1b2e4b]">
                    <div class="px-5" x:data="typelist">
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="heading">
                                            <h2 class="m-0">Add Types</h2>
                                        </div>
                                        <div class="p-5">
                                            <form enctype="multipart/form-data" id="signupForm" class="needs-validation"
                                                novalidate="" method="POST" action="{{ route('types.store') }}">
                                                @csrf
                                                <div class="grid grid-cols-1 gap-4 pt-5">
                                                    <div>
                                                        <label for="name">Name</label>
                                                        <input id="Name" type="text" placeholder="Type Name" name="name"
                                                ...
                                                <div class=" flex justify-end items-center mt-3">
                                                    ...
                                                    <button type="submit" class="btn btn-primary ltr:ml-4 rtl:mr-4">Create</button>
                                                </div>
                                            </form>
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        class="panel border-0 p-0 rounded-lg overflow-hidden my-8 w-full max-w-lg" style="border-radius:.5rem;border-width: 0;">
                                        <div class="heading">
                                            <h2 class="m-0">Edit Types</h2>
                                        </div>
                                        <div class="p-5">
                                            <form enctype="multipart/form-data" id="type_edit_form" class="needs-validation" method="POST"
                                                ...
                                                <div class="grid grid-cols-1 gap-4 pt-5">
                                                    ...
                                                    <div style="display: flex; align-items: center; gap: 10px;">
                                                        <img src="" style="width: 50px; height: 50px; margin: 0; border-radius: 5px; object-fit: cover;"
                                                            id="image-input" class="w-24 h-24 mb-2">
                                                        <input class="form-input" id="Image" type="file" name="image"
                                                            accept="image/*" @change="handleIconChange" />
                                                    </div>
...
<script>
    ...
    $(document).ready(function () {
        var ajax_url = "{{action('Rest\TypeController@index')}}";
        ...
        $(document).on('click', "table#facility_table tbody tr td", function() {
            ...
            $(document).on('click', 'button.edit-btn', function() {
                ...
                const edit_url = "{{action('Rest\TypeController@show', ['ID'])}}".replace('ID', id);
                $.ajax({
                    method: "GET",
                    url: edit_url,
                    dataType: "json",
                    success: function (result) {
                        console.log(result)
                        $('form#type_edit_form #edit-id').val(result.id);
                        $('form#type_edit_form').find('#name').val(result.name);
                        $('form#type_edit_form').find('img').attr("src", result.icon);
                        if(result.status == 'Active')
                        {
                            $('form#type_edit_form').find('#status-input').prop('checked', true);
                        }
                        else
                        {
                            $('form#type_edit_form').find('#status-input').prop('checked', false);
                        }
                    }
                });
            });
        ...
        $(document).on('change', '#check-box', function()
        {
            ...
            {   
                ...
                $(document).on('click', 'button.delete-button', function() {
                    const delete_url = "{{action('Rest\TypeController@delete')}}";
                    $.ajax({
                        method: "GET",
                        url: delete_url,
                        dataType: "json",
