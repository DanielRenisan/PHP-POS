...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    <div>
        ...
        <div class="grid grid-cols-1 gap-4 pt-5">
            <div x-data="emdestination">
                <div class="panel border-[#e0e6ed] px-0 dark:border-[#1b2e4b]">
                    <div class="px-5" x:data="emdestination">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div class="mb-5 flex items-center gap-2">
                                @can('em-destination.create')
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="heading">
                                            <h2 class="m-0">Add EM Destination</h2>
                                        </div>
                                        <div class="p-5">
                                            <form id="signupForm" class="needs-validation" novalidate="" method="POST"
                                                ...
                                                <div class="grid grid-cols-1 gap-4 pt-5">
                                                    <div>
                                                        ...
                                                        <input id="Name" type="text" placeholder="Destination Name" name="name"
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        class="panel border-0 p-0 rounded-lg overflow-hidden my-8 w-full max-w-lg">
                                        <div class="heading">
                                            <h2 class="m-0">Edit EM Destination</h2>
                                        </div>
                                        <div class="p-5">
                                            <form id="des_edit_form" class="needs-validation" novalidate="" method="POST"
                                                action="{{ route('destination.update') }}">
                                                @csrf
                                                ...
                                                <div class=" flex justify-end items-center mt-3">
                                                    <button type="submit" class="btn btn-outline-danger discard-btn"
                        ...
                        <table id="example" class="custom-table display nowrap" style="width:100%">
                            ...
                            <tbody>
                                @foreach($destinations as $destination)
                                    <tr id="check-tr" data-id="{{ $destination['id'] }}">
                                        <td>
                                        ...
                                        <td style="text-align:center">
                                            ...
                                            <span class="badge badge-outline-success">{{ $destination['status'] }}</span>
                                            @else
                                            <span class="badge badge-outline-danger">{{ $destination['status'] }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="11">{{$destinations->links('vendor.pagination.tailwind')}}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
...
//Category list
    ...
    document.getElementById('pagination').onchange = function() { 
        window.location = "{!! $destinations->url(1) !!}&items=" + this.value; 
    };
    ...
    $(document).ready(function () {
        $(document).on('click', "table#example tbody tr td", function() {
            ...
            $(document).on('click', 'button.edit-btn', function() {
                ...
                const edit_url = "{{action('Rest\EMDestinationController@show', ['ID'])}}".replace('ID', id);
                $.ajax({
                    method: "GET",
                    url: edit_url,
                    dataType: "json",
                    success: function (result) {
                        $('form#des_edit_form #type_id').val(result.id);
                        $('form#des_edit_form').find('#Name').val(result.name);
                        if(result.status == 'Active')
                        {
                            $('form#des_edit_form').find('#status-input').prop('checked', true);
                        }
                        else
                        {
                            $('form#des_edit_form').find('#status-input').prop('checked', false);
                        }
                    }
                });
            });
        ...
        $(document).on('change', '#check-box', function()
        {
            ...
            {   
                ...
                const delete_url = "{{action('Rest\EMDestinationController@delete')}}";
                $.ajax({
                    method: "GET",
                    url: delete_url,
                    dataType: "json",
    ...
    $(document).on('click', 'discard-btn', function() {
        $('form#des_edit_form #type_id').val(' ');
        $('form#des_edit_form').find('#Name').val(' ');
    });
</script>
@endsection