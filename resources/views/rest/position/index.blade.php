                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        class="panel border-0 p-0 rounded-lg overflow-hidden my-8 w-full max-w-lg">
                                        <div class="heading">
                                            <h2 class="m-0">Edit Position</h2>
                                        </div>
                                        <div class="p-5">

                                            <form id="edit_position_form" class="needs-validation" novalidate="" method="POST"
                                                action="{{ route('position.update') }}">
                                                @csrf
                        ...
                        <table id="example" class="custom-table display nowrap" style="width:100%">
                            ...
                            <tbody>
                                @foreach($positions as $position)
                                    <tr id="check-tr" data-id="{{ $position['id'] }}">
                                        <td>
                                        ...
                                        <td style="text-align:center">
                                            ...
                                            <span class="badge badge-outline-success">{{ $position['status'] }}</span>
                                            @else
                                            <span class="badge badge-outline-danger">{{ $position['status'] }}</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="11">{{$positions->links('vendor.pagination.tailwind')}}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
...
<script>
    ...
    document.getElementById('pagination').onchange = function() { 
        window.location = "{!! $positions->url(1) !!}&items=" + this.value; 
    }; 
    ...
    $(document).ready(function () {
        $(document).on('click', "table#example tbody tr td", function() {
            ...
            $(document).on('click', 'button.edit-btn', function() {
                ...
                const edit_url = "{{action('Rest\PositionController@show', ['ID'])}}".replace('ID', id);
                $.ajax({
                    method: "GET",
                    url: edit_url,
                    dataType: "json",
                    success: function (result) {
                        $('form#edit_position_form #type_id').val(result.id);
                        $('form#edit_position_form').find('#Name').val(result.name);
                        if(result.status == 'Active')
                        {
                            $('form#edit_position_form').find('#status-input').prop('checked', true);
                        }
                        else
                        {
                            $('form#edit_position_form').find('#status-input').prop('checked', false);
                        }
                    }
                });
            });
        ...
        $(document).on('change', '#check-box', function()
        {
            ...
            {   
                ...
                const delete_url = "{{action('Rest\PositionController@delete')}}";
                $.ajax({
                    method: "GET",
                    url: delete_url,
                    dataType: "json",
    ...
    $(document).on('click', 'discard-btn', function() {
        $('form#edit_position_form #type_id').val(' ');
        $('form#edit_position_form').find('#Name').val(' ');
    });
</script>
@endsection