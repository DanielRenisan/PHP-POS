...
<div class="animate__animated p-6" :class="[$store.app.animation]">
    <div>
        ...
        <div class="grid grid-cols-1 gap-4 pt-5">
            <div x-data="brandlist">
                <div class="panel border-[#e0e6ed] px-0 dark:border-[#1b2e4b]">
                    <div class="px-5" x:data="{brandlist, checkAllCheckbox: false, selectedRows: [] }">
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="heading">
                                            <h2 class="m-0">Add Brand</h2>
                                        </div>
                                        <div class="p-5">
                                            <form enctype="multipart/form-data" id="signupForm" class="needs-validation"
                                                method="POST" action="{{ route('brand.store') }}">
                                                @csrf
                                                <div class="grid grid-cols-1 gap-4 pt-5">
                                                    <div>
                                                        <label for="name">Name</label>
                                                        <input id="name" type="text" placeholder="Brand Name" name="name"
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        class="panel border-0 p-0 rounded-lg overflow-hidden my-8 w-full max-w-lg"  style="border-radius:.5rem;border-width: 0;">
                                        <div class="heading">
                                            <h2 class="m-0">Edit Brand</h2>
                                        </div>
                                        <div class="p-5">
                                            <form enctype="multipart/form-data" id="brand_edit_form" class="needs-validation" method="POST"
                                                action="{{ route('brand.update') }}">
                                                @csrf
...
<script>
    ...
    $(document).ready(function () {
        var ajax_url = "{{action('Rest\BrandController@index')}}";
        ...
        $(document).on('click', "table#facility_table tbody tr td", function() {
            ...
            $(document).on('click', 'button.edit-btn', function() {
                ...
                const edit_url = "{{action('Rest\BrandController@show', ['ID'])}}".replace('ID', id);
                $.ajax({
                    method: "GET",
                    url: edit_url,
                    dataType: "json",
                    success: function (result) {
                        console.log(result)
                        $('form#brand_edit_form #edit-id').val(result.id);
                        $('form#brand_edit_form').find('#name').val(result.name);
                        $('form#brand_edit_form').find('img').attr("src", result.icons);
                        if(result.status == 'Active')
                        {
                            $('form#brand_edit_form').find('#status-input').prop('checked', true);
                        }
                        else
                        {
                            $('form#brand_edit_form').find('#status-input').prop('checked', false);
                        }
                    }
                });
            });
        ...
        $(document).on('change', '#check-box', function()
        {
            ...
            {   
                ...
                $(document).on('click', 'button.delete-button', function() {
                    const delete_url = "{{action('Rest\BrandController@delete')}}";
                    $.ajax({
                        method: "GET",
                        url: delete_url,
                        dataType: "json",
