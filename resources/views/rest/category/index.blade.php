        ...
        <div class="grid grid-cols-1 gap-4 pt-5">
            <div x-data="categoryList">
                <div class="panel border-[#e0e6ed] px-0 dark:border-[#1b2e4b]">
                    <div class="px-5" x:data="{categoryList, checkAllCheckbox: false, selectedRows: [] }">
                                    ...
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="p-5">
                                            <form enctype="multipart/form-data" id="signupForm" class="needs-validation"
                                                method="POST" action="{{ route('categeory.store') }}">
                                                @csrf
                                                <div style="display: flex; flex-direction: column; gap: 10px;">
                                                    ...
                                                    <div>
                                                        ...
                                                        <input id="Name" type="text" placeholder="Category Name" name="name"
                                                            class="form-input" required />
                                                    </div>
                                                    <!-- Category Selection -->
                                                    <div>
                                                        <label for="mainCategory">Main Category</label>
                                                        <select class="form-select text-white-dark" name="parent" id="parent">
                                                            <option value="" selected>Select Parent</option>
                                                            @foreach ($categeory_list as $pc)
                                                            <option name="SelectParent" class="pro-type" value="{{ $pc->id }}">
                                                                {{ $pc->name }}</option>
                                                            @endforeach
                                                    ...
                                                    <div style="display: flex; align-items: center; gap: 10px; object-fit: cover;">
                                                        <!-- <img style="width: 50px; height: 50px; margin: 0; border-radius: 5px;"
                                                            alt="Current Icon" class="w-24 h-24 mb-2"> -->
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto" :class="open && '!block'">
                                <div class="flex items-start justify-center min-h-screen px-4" @click.self="open = false">
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="p-5">
                                            <form enctype="multipart/form-data" id="cat_edit_form" class="needs-validation" novalidate="" method="POST"
                                                action="{{ route('categeory.update') }}">
                                                @csrf
                                                <input type="hidden" name="id" id="edit-id">
                                                <div>
                                                    <label for="code">Code</label>
                                                    <input id="code" type="text" name="code" class="form-input"
                                                        required />
                                                </div>
                                                <div>
                                                    <label for="name">Name</label>
...
<script>
    ...
    $(document).ready(function () {
        var ajax_url = "{{action('Rest\ProductCategoryController@index')}}";
        ...
        $(document).on('click', "table#facility_table tbody tr td", function() {
            ...
            $(document).on('click', 'button.edit-btn', function() {
                ...
                const edit_url = "{{action('Rest\ProductCategoryController@show', ['ID'])}}".replace('ID', id);
                $.ajax({
                    method: "GET",
                    url: edit_url,
                    dataType: "json",
                    success: function (result) {
                        ...
                        $('form#cat_edit_form').find('#code').val(result.code);
                        $('form#cat_edit_form').find('#name').val(result.name);
                        $('form#cat_edit_form').find('#parent').val(result.parent_id);
                        $('form#cat_edit_form').find('img').attr("src", result.icon);
                        if(result.status == 'Active')
                        {
                            $('form#cat_edit_form').find('#status-input').prop('checked', true);
                        }
                        else
                        {
                            $('form#cat_edit_form').find('#status-input').prop('checked', false);
                        }
                    }
                });
            });
        ...
        $(document).on('change', '#check-box', function()
        {
            ...
            {   
                ...
                $(document).on('click', 'button.delete-button', function() {
                    const delete_url = "{{action('Rest\ProductCategoryController@delete')}}";
                    $.ajax({
                        method: "GET",
                        url: delete_url,
                        dataType: "json",
