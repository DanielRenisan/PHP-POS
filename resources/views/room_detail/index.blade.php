    ...
    <div>
        <ul class="flex space-x-2 rtl:space-x-reverse">
            ...
            <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                <span>Room Facilities Details</span>
            </li>
        </ul>
                    ...
                    <div class="px-5" x:data="categoryList">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div class="mb-5 flex items-center gap-2">
                                @can('room-details.delete')
                                    <button type="button" class="btn btn-danger gap-2 delete-button" @click="deleteRow()" data-href = "{{action('RoomDetailController@destroy')}}">
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                                ...
                                <div class="flex items-start justify-center min-h-screen px-4"
                                    ...
                                    <div x-transition x-transition.duration.300
                                        ...
                                        <div class="heading">
                                            <h2 class="m-0">Add Facility Detail</h2>
                                        </div>
                                        <div class="p-5">
                                        {!! Form::open(['url' => action('RoomDetailController@store'), 'method' => 'post', 'class' => 'space-y-5', 'id' => 'call_add_form'  ,'files' => true]) !!}
                                        
                                                <div>
                                                    <label for="code">Facility Type</label>
                                                    {!! Form::select('room_facility_id', $facilities, null, ['class' => 'form-select text-white-dark', 'id'=>'seachable-select',
                                                ...
                                                <div style="display: flex; align-items: center; gap: 10px;">
                                                    <img style="width: 50px; height: 50px; margin: 0; border-radius: 5px; object-fit: cover;"
                                                        :src="itemToEdit.image" alt="Current Icon" class="w-24 h-24 mb-2">
                            ...
                            <div class="fixed inset-0 bg-[black]/60 z-[999] hidden overflow-y-auto"
                                ...
                                <div class="flex items-start justify-center min-h-screen px-4"
                                    ...
                                    <div x-transition x-transition.duration.300
                                        class="panel border-0 p-0 rounded-lg overflow-hidden my-8 w-full max-w-lg">
                                        <div class="heading">
                                            <h2 class="m-0">Edit Detail</h2>
                                        </div>
                                        <div class="p-5">
                                            {!! Form::open(['url' => action('RoomDetailController@update', ['ID']),'class'=>'space-y-5', 'method' => 'PUT', 'id' => 'call_edit_form' ,'files' => true ]) !!}
                                                <div>
                                                    <label for="code">Facility Type</label>
                                                    {!! Form::select('room_facility_id', $facilities, null, ['class' => 'form-select text-white-dark', 'id'=>'facility_id',
                                                                                'placeholder' => __('Select Type'),'required','x-model'=>'itemToEdit.facility_id']); !!}
                                                </div>
                                                <input id="edit-id" type="hidden" x-model="itemToEdit.id">
                                                <div>
                                                    {!! Form::label('name', __('Facility') . ':*') !!}
                                                    {!! Form::text('name', null, ['class' => 'form-input','required','x-model'=>'itemToEdit.name', 'id' => 'name-edit']); !!}
                                                </div>
                                                <div style="display: flex; align-items: center; gap: 10px;">
                                                    <img style="width: 50px; height: 50px; margin: 0; border-radius: 5px; object-fit: cover;"
                                                        :src="itemToEdit.image" alt="Current Icon" class="w-24 h-24 mb-2">
                                                    <input class="form-input" id="edit-image" type="file" name="image"
                                                        accept="image/*" @change="handleIconChange" />
                                                </div>
                            ...
                            <tbody>
                                <template x-for="(item, index) in paginatedFilteredItems" :key="item.id">
                                    <tr>
                                        ...
                                        <td>
                                            <span x-text="item.facility"></span>
                                        </td>
                                        <td>
                                            <img style="border-radius: 5px; object-fit: cover;" :src="item.image"
                                            alt="Product Icon" class="w-8 h-8" />
                                        </td>
                                        @can('room-details.update')
                                        <td>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('categoryList', () => ({
                ...
                handleIconChange(event) {
                    ...
                    if (file) {
                        ...
                        if (!allowedFormats.includes(file.type)) {
                            ...
                            reader.onload = () => {
                                this.itemToEdit.image = reader.result; // Update the icon with the new image data
                            };
                        }
                    }
                },
                ...
                editCategory() {
                    ...
                    var room_facility_id = $("#facility_id").val();
                    var photo = $('#edit-image').prop('files')[0];   
                    formData.append('image', photo);
                    formData.append('name', name);
                    formData.append('room_facility_id', room_facility_id);
