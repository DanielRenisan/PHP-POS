...
<body class="relative overflow-x-hidden font-nunito text-sm font-normal antialiased"
    ...
    <script src="{{asset('asset/js/pos.js')}}"></script>
...
<script src="{{ asset('AdminLTE/plugins/jQuery/jquery-2.2.3.min.js') }}"></script>
    ...
    initializeSocket = function() {
        ...
        localStorage.setItem("selectedCustomer", '{{$walk_customer["id"]}}');
        localStorage.setItem("selectedEmployee", '{{auth()->user()->id}}');
        ...
        Alpine.data("form", () => ({
            ...
            init() {
                // Watch for changes that affect the total and update payment amount
                this.$watch('seviceCharge', () => {
                    if (this.cashPopup) {
                        this.payments[0].amount = this.calculateGrandTotal();
                    }
                    if (this.multyPaymentPopup) {
                        const newGrandTotal = this.calculateGrandTotal();
                        this.multytotalAmount = newGrandTotal;
                        
                        // Update the first payment amount to match new total
                        if (this.multyPayments.length > 0) {
                            this.multyPayments[0].amount = newGrandTotal;
                        }
            ...
            canceledData: <?php echo $canceled_data; ?>,
            isActive: false,
            
            cuisines: <?php echo $brands; ?>,
            brands: <?php echo $brands; ?>,
            ...
            buttons: [
                ...
                {
                    name: "Add Discount Percentage",
                    icon: "fas fa-percent",
                    id: "",
                },
                {
                    name: "Add Discount Ammount",
            ...
            stocks: [],
            dineInButtons: <?php echo $tables; ?>,

            roomsButtons: <?php echo $rooms; ?>,
            ...
            addToMyProducts(product) {
                if (this.selectedCustomer == 'All') {
                    // Display an alert if the selected customer is not set
                    showMessage("Please select a customer before adding the product.");
                    return; // Stop further execution
                }
                // if (this.selectedLocation == 'All') {
                //     // Display an alert if the selected customer is not set
                //     showMessage("Please select a location before adding the product.");
                //     return; // Stop further execution
                // }

                if (this.selectedEmployee == 'All') {
                    // Display an alert if the selected customer is not set
                    showMessage("Please select a emplyee before adding the product.");
                    return; // Stop further execution
                }

                if (
                    this.selectedRoomInButtons == null &&
                    this.selectedDineInButtons == null && this.selectedOrderType == null
                ) {
                    // Display an alert if neither room nor table is selected
                    // showMessage("Please select an order type before adding the product.");
            ...
            openCashPop()
            {
                ...
                if (allSelectProducts.length <= 0)
                {
                    ...
                    // Calculate current total and set it directly to payments array
                    const grandTotal = this.calculateGrandTotal();
                    this.payments[0].amount = grandTotal;  // ← This line was added
                    this.totalAmount = grandTotal;
                    this.remainingAmount = grandTotal;
                    this.cashPopup = true;
                }
            },
            openMultyPaymentPop()
            {
                ...
                else
                {
                    ...
                    const currentTotal = this.calculateGrandTotal();
                    
                    // Update the payment amounts
                    this.multytotalAmount = currentTotal;
                    this.remainingMultyAmount = currentTotal;
                    
                    // Reset payments array with current total
                    this.multyPayments = [
                        {
                            amount: currentTotal,
                            method: "cash",
            ...
            updateServiceChargeAndPayments() {
                const newGrandTotal = this.calculateGrandTotal();
                const previousTotal = this.multytotalAmount;
                const difference = newGrandTotal - previousTotal;
                
                // Update total amount
                this.multytotalAmount = newGrandTotal;
                
                // Update the remaining amount calculation
                this.remainingMultyAmount = this.calculateRemainingMultyAmount();
            },
            ...
            getCashPayment() {
                ...
                // Check if this is a cash payment (from cash popup)
                if (this.cashPopup) {
                    // For cash payments, create clean form data
                    let formData = $('form#pos_add_form').serializeArray();
                    
                    // Remove any existing payment[0][amount] entries
                    formData = formData.filter(item => item.name !== 'payment[0][amount]');
                    
                    // Add the correct cash payment amount
                    formData.push({
                    ...
                    const paymentMethodIndex = formData.findIndex(item => item.name === 'payment[0][method]');
                    if (paymentMethodIndex !== -1) {
                        formData[paymentMethodIndex].value = 'cash';
                    } else {
                        formData.push({
                    ...
                    // For multi-payment or other methods, use original logic
            ...
            heldInvoices: <?php echo $transaction_draft ?>,
            heldOrders: <?php echo $transaction_data ?>,
            printOrders: [],
            kotDisplayOrders: <?php echo $orders ?>,
            ...
            performAction(actionName, shortcut) {
                switch (actionName) {
                    ...
                    case "Delivery":
                        ...
                        break;
                    case "Add Discount Percentage":
                        if (this.myProducts.length > 0) {
                            this.disPercentagePopup = true;
                        }
                        break;
                    case "Add Discount Ammount":
                        if (this.myProducts.length > 0) {
         ...
         // This method returns the background color based on order status
            ...
            makeQuickPayment(amount) {
                // Set the payment amount directly (don't add to existing)
                this.payments[0].amount = amount;
                this.remainingAmount = this.totalAmount - amount;
            },
            ...
            updateTotalAmount(event, index) {
                ...
                this.multyPayments[index].amount = Math.max(0, amount);
                this.remainingMultyAmount = this.calculateRemainingMultyAmount();
            },
    ...
    existingCustomers: <?php echo $existing_customers; ?>,
